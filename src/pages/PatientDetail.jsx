...
export default function PatientDetail() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-surface-high flex gap-6">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
      ...
      <div className="card-pad">
        <div className="flex items-center justify-between mb-4">
          <h2 className="h3 flex items-center gap-2">
            <CalendarDaysIcon className="w-5 h-5 text-primary" /> جدول اليوم المركّز
          </h2>
          <span className="label-caps">{can("patients.edit") ? "عرض متزامن" : "عرض للقراءة فقط"}</span>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { time: "9:00 ص", name: "سارة العتيبي", reason: "ما بعد العملية", active: true },
            { time: "10:30 ص", name: "فيصل الحربي", reason: "مراجعة تحاليل" },
            { time: "11:15 ص", name: "ليلى الغامدي", reason: "استشارة" },
            { time: "1:45 م", name: "جمال الحربي", reason: "متابعة" },
          ].map((slot, i) => (
            <motion.div
              key={i}
              ...
              className={`p-4 rounded-lg border ${
                slot.active ? "border-primary bg-primary-soft/40" : "border-surface-high bg-surface-low/60"
              }`}
            >
              <div className="label-caps text-primary">{slot.active ? "حالي" : slot.time}</div>
              {slot.active && <div className="text-xs text-ink-mute font-semibold mt-0.5">{slot.time}</div>}
              <div className="mt-1 font-semibold text-ink">{slot.name}</div>
              <div className="text-xs text-ink-mute">{slot.reason}</div>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="card-pad">
          ...
          <div className="flex items-center gap-1 mb-3 text-ink-mute">
            ...
            <ToolBtn><ItalicIcon className="w-4 h-4" /></ToolBtn>
            <ToolBtn><ListBulletIcon className="w-4 h-4" /></ToolBtn>
            <ToolBtn><CameraIcon className="w-4 h-4" /></ToolBtn>
            <ToolBtn><MicrophoneIcon className="w-4 h-4" /></ToolBtn>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="card-pad">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {patient.allergies.length === 0 && <span className="text-sm text-ink-mute">لا توجد حساسيات معروفة.</span>}
        ...
        <div className="card-pad">
          ...
          <div className="mt-3 space-y-3">
            {patient.meds.length === 0 && <div className="text-sm text-ink-mute">لا توجد وصفات فعّالة.</div>}
